...
const MainPage = ({ params }: { params: { code: string } }) => {
  ...
  const getVote = async (code: string) => {
    try {
      const { data } = await axios.get(`${process.env.NODE_ENV !== 'development' ? 'https://voting-4gcxmnhfj-bagussatoto.vercel.app/' : 'http://localhost:3000'}/api/votes/${code}`);
      setVote(data.result);
    } catch (error: any) {
  ...
  const getParticipantVote = async (code: string) => {
    try {
      const { data } = await axios.get(`${process.env.NODE_ENV !== 'development' ? 'https://voting-4gcxmnhfj-bagussatoto.vercel.app/' : 'http://localhost:3000'}/api/participant/${code}`);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.post(`${process.env.NODE_ENV !== 'development' ? 'https://voting-4gcxmnhfj-bagussatoto.vercel.app/' : 'http://localhost:3000'}/api/participant`, {
