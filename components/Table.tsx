...
const Table = () => {
  ...
  const handleDelete = (id: string) => {
    ShowModal({
      ...
      async onPositiveClick() {
        try {
          await axios.delete(`${process.env.NODE_ENV !== 'development' ? 'https://voting-4gcxmnhfj-bagussatoto.vercel.app/' : 'http://localhost:3000'}/api/votes/${id}`);
  ...
  const getVotes = async () => {
    setLoading(true);
    try {
      const result = await axios.get(`${process.env.NODE_ENV !== 'development' ? 'https://voting-4gcxmnhfj-bagussatoto.vercel.app/' : 'http://localhost:3000'}/api/votes`);
